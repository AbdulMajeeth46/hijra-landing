...
<nav class="fixed top-0 left-0 w-full z-50 font-['Urbanist'] antialiased transition-all duration-300 ease-out {isScrolled ? 'h-20' : 'h-24'}">
  
  <div 
    class="absolute inset-0 w-full h-full border-b transition-opacity duration-300 ease-out pointer-events-none bg-white border-gray-100
  ...
  <div class="relative max-w-[1400px] mx-auto px-6 md:px-12 h-full flex items-center justify-between">
    ...
    <div class="hidden md:flex items-center gap-1 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
      {#each siteConfig.nav as link}
        <a 
          href={link.href} 
          class="relative px-5 py-2 rounded-full text-[15px] font-medium text-gray-500 hover:text-[#1B1B1B] transition-all duration-200 hover:bg-gray-50"
        >
          {link.label}
        </a>
      {/each}
    ...
    <div class="flex items-center gap-4 shrink-0 z-50 relative">
      ...
      <a 
        href="/login" 
        class="hidden sm:flex items-center justify-center px-6 py-2.5 rounded-full text-[14px] font-bold tracking-tight border border-gray-200 bg-white text-[#1B1B1B]
               transition-all duration-200 ease-out
               shadow-none
               hover:-translate-y-0.5 hover:-translate-x-0.5 hover:shadow-[3px_3px_0px_0px_#1B1B1B] hover:border-[#1B1B1B]
  ...
  {#if isMobileMenuOpen}
    <div class="fixed inset-0 bg-white z-40 flex flex-col items-center justify-center pt-24 pb-10 md:hidden animate-fade-in overflow-y-auto">
      <div class="flex flex-col gap-8 text-center w-full px-6">
        ...
        <a href="/login" class="text-xl font-medium text-gray-500 hover:text-black transition-colors" onclick={toggleMobileMenu}>
            Log In
        </a>
        
        <a href="/contact" class="text-xl font-bold text-[#00B77A]" onclick={toggleMobileMenu}>
            Get Started
        </a>
      </div>
    </div>
